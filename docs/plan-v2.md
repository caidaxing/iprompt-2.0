...
## 9. 追加交付:模板库 + 技能占位(2026-09-20)

- **模板库 `/templates`**:13 分类 / 47 个模板块 / 48 条防坑指南,内容逆向整理自上游 `docs/templates.md`(MIT,已致谢)。左侧分类锚点导航 + 滚动高亮,每个模板块(文本填空 / Agent JSON)独立一键复制
- **技能中心 `/skills` 占位页**:两个技能卡(案例风格库 / 工业级模板包)标注"整理中",附上游技能包参考链接;正式打包分发留待后续版本
- 导航新增「模板库」「技能」入口

## 10. 模板库视觉重构(实测对齐上游官网,2026-09-20)

经浏览器实测上游官网 /#templates 后重构:
- **卡片画廊**:22 张模板卡(封面大图 + Template 编号徽章 + 分类眉题 + 中文标题 + 一句话描述 + 标签 chips),三列网格
- **点击卡片弹详情浮层**:左侧封面大图,右侧徽章/标题/描述/标签/适用场景框/核心指引/需要避免,「复制模板 Prompt」一键复制其官网同款结构化提示词(主体/场景/构图/风格/文本/细节/输出+约束+避坑),附上游文档直达链接
- 数据源修正:卡片数据实际来自上游 `data/style-library.json` 的 templates 数组(22 条双语),而非 docs/templates.md;封面图映射其仓库原图
- 原有的 47 块分类填空模板详表保留在页面下方("完整填空模板与防坑指南"),锚点导航不变
