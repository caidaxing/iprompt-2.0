      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div>
          {resultPath ? (
            ...
            <div>
              <div className="border border-dashed border-line rounded-sm min-h-40 flex items-center justify-center mb-4">
              ...
              <p className="text-xs text-ink-mute mb-2">从这些案例开始 →</p>
              <div className="grid grid-cols-3 gap-3">
                {(cases ?? []).slice(0, 3).map((c) => (
                  <button
                    ...
                    className={`text-left border rounded-sm overflow-hidden transition-all hover:-translate-y-0.5 hover:shadow-md ${
                      selected?.id === c.id ? "border-ink" : "border-line hover:border-ink"
                    }`}
                  >
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={c.image} alt={c.title} className="w-full aspect-square object-cover" />
                    <span className="block px-2 py-1.5 text-[11px] text-ink-soft truncate">{c.title}</span>
                  </button>
                ))}
              </div>
            </div>
